...
class ParseWB:
    ...
    @staticmethod
    def __create_csv():
        if not os.path.isfile("../../data/dataset_old.csv"):
            with open("../../data/dataset_old.csv", mode="w", newline="", encoding="utf-8") as file:
    ...
    def __feedback(item_model: Items):
        for product in item_model.products:
            ...
            if res.status_code == 200:
                item = Item.parse_obj(product)
                feedback = Feedback.parse_obj(res.json())
                if feedback.feedbacks is not None:
                    for f in feedback.feedbacks:
                        f = Text.parse_obj(f)
                        with open("../../data/dataset_old.csv", mode="a", newline="", encoding="utf-8") as file:
                            writer = csv.writer(file)
                            writer.writerow([
                                item.name,
                                item.brand,
