...
import csv
import os
from models import Items, Category, NewFeedbacks
...
class Parser:
        ...
        for root in self.roots:
            ...
            if not response:
                ...
                if not response:
                    continue
            if response.json():
                feedbacks = NewFeedbacks.model_validate(response.json()).feedbacks
                if feedbacks:
                    for f in feedbacks:
                        if f.text != "":
                            if len(self.remove_emojis_and_english(f.text).split()) >= 30:
                                with open("dataset30.csv", mode="a", newline="", encoding="utf-8") as file:
    ...
    def __init_roots(self):
        ...
        for params in self.request_params:
            ...
            if response:
                products = Items.model_validate(response.json()["data"]).products
                for pr in products:
    ...
    def __create_csv():
        ...
        if not os.path.isfile("dataset30.csv"):
            with open("dataset30.csv", mode="w", newline="", encoding="utf-8") as file:
                writer = csv.writer(file)
